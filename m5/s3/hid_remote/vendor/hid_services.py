...
# Local: avoid synchronous console logging inside BLE callbacks.
...
class Advertiser:
    ...
    # Start advertising, or update the interval while already advertising.
    def start_advertising(self, interval_us=100000):
        if not self.advertising or interval_us != self.interval_us:
...
class HumanInterfaceDevice(object):
    ...
    def __init__(self, device_name="Generic HID Device"):
        ...
        self.mitm = False  # Local: Just Works media remote; separate from LE SC.
        ...
        if event == _IRQ_CENTRAL_CONNECT:                                                                               # Central connected.
            ...
            if value is None:                                                                                           # If value is empty, and
                if self.secrets.has_secret(sec_type, key):                                                              # If key is known then
                    ...
                    # Local: never log bonding material.
                    return True
                else:
                    # Local: never log bonding material.
                    return False
    ...
    def start(self):
        if self.device_state is HumanInterfaceDevice.DEVICE_STOPPED:
            ...
            self._ble.config(mitm=self.mitm)                                                                       # Require man in the middle protection.
